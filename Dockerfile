...
RUN apt-get update && \
    ...
    rm -rf /var/lib/apt/lists/* /cuda-repo-wsl-ubuntu-11-8-local_11.8.0-1_amd64.deb
...
RUN R --no-save --no-restore -e "install.packages(c('remotes', 'audio', 'abseil', 'torch'))" && \
...
CMD ["bash", "-c", "R --no-save --no-restore -e 'if (!requireNamespace(\"audio.whisper\", quietly = TRUE)) { remotes::install_github(\"bnosac/audio.whisper\") }' && exec /init"]
